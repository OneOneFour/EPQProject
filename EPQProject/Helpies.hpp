...
extern float getArgument(sf::Vector2f A);//gets angle from the vertical
extern float getArgumentH(sf::Vector2f A);//gets angle from horizontal 
...
extern float radToDegree(float angle);//Converts from radians to degrees
extern float degreeToRad(float angle);//converts angle from degrees to radians 